...
def analyze(text):
    ...
    # OCR mistake fixing (OCR often confuses 0 and O, or 1 and I)
    t = t.replace("OX", "0X").replace("0O", "00")
    
    keys = sorted(PANIC_DATABASE.keys(), key=len, reverse=True)
    results = [{"code": k, **PANIC_DATABASE[k]} for k in keys if k in t]
    
    # Secondary pass for fuzzy HEX codes via Regex
    if not results:
    ...
    if not results:
        ...
        return "❌ *لم يتم اكتشاف أكواد معروفة*\nتأكد من وضوح الصورة المرفوعة."
        
    report = f"🔍 *نتيجة فحص PincFull — {len(results)} عطل:*\n\n"
    for r in results:
        report += f"🔴 *العطل:* {r['f']}\n🔧 *القطعة:* {r['p']}\n✅ *الحل:* {r['s']}\n─────────────\n"
    return report
