...
async def check_sub(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        if member.status in ['left', 'kicked']:
            keyboard = [[InlineKeyboardButton("اشترك في القناة الآن 🔔", url=f"https://t.me/{CHANNEL_USERNAME[1:]}")]]
        ...
        await update.message.reply_text(
            f"⚠️ *تنبيه للمطور:*\nميزة الاشتراك الإجباري معطلة الآن لأن البوت **ليس مشرفاً (Admin)** في القناة `https://t.me/{CHANNEL_USERNAME[1:]}`.\nيرجى الدخول لقناتك وإضافة البوت كمشرف لكي يرى المشتركين.",
            parse_mode='Markdown'
        )
        return False
