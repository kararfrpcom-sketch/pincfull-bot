...
CHANNEL_USERNAME = "@Ai_pro26" # Username for checking membership (Bot must be Admin)
INVITE_LINK = "https://t.me/+TnTEQjUhUfozOTMy" # Your actual invite link
...
def analyze(text):
    ...
    for r in results:
        report += f"🔴 *العطل:* {r['f']}\n🔧 *القطعة:* {r['p']}\n✅ *الحل:* {r['s']}\n─────────────\n"
    report += "\n_[PincFull Pro | Dev: kararAhmed](https://t.me/Kguitaraa)_"
    return report
...
async def check_sub(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        if member.status in ['left', 'kicked']:
            keyboard = [[InlineKeyboardButton("اشترك في القناة الآن 🔔", url=INVITE_LINK)]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text(
                f"⚠️ *عذراً عزيزي!*\n\nلا يمكنك استخدام البوت أو الموقع إلا بعد الاشتراك في القناة الرسمية للمطور:\n{INVITE_LINK}\n\nيرجى الاشتراك ثم اضغط /start مرة أخرى للحصول على رمز الدخول للموقع.",
        ...
        await update.message.reply_text(
            f"⚠️ *تنبيه للمطور:*\nميزة الاشتراك الإجباري معطلة الآن لأن البوت **ليس مشرفاً (Admin)** في القناة.\n\nتأكد من إضافة البوت في القناة كمسؤول (Admin) لكي يستطيع فحص المشتركين.\nرابط القناة الذي تم ضبطه: {INVITE_LINK}",
            parse_mode='Markdown'
        )
        return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Save password to Firebase RTDB
        requests.put(f"{DB_URL}/passwords/{new_pass}.json", json={"user_id": user_id, "status": "active"}, timeout=5)
        
        # Increment real-time users count
        try:
            count_req = requests.get(f"{DB_URL}/users_count.json", timeout=5)
            curr_count = count_req.json()
            if curr_count is None: curr_count = 15465
            requests.put(f"{DB_URL}/users_count.json", json=(curr_count + 1), timeout=5)
        except: pass
        
        msg = (
            "💎 *مرحباً بك في أداة PincFull Pro المركزية*\n\n"
            f"🔑 *لقد تم إعطاؤك رمز الدخول الخاص بك مجاناً:*\n`{new_pass}`\n\n"
            "⚠️ يحق لك استخدام هذا الرمز في متصفحك فقط ولا يُسمح لك بمشاركته.\n\n"
            "🌐 *افتح الموقع من هنا وأدخل الرمز لاستخدام الأداة:*\n"
            "👉 [pincfull.web.app](https://pincfull.web.app)\n\n"
            "أو إذا أردت فحص الصور والملفات هنا مباشرة، فأرسلها للبوت الآن!\n\n"
            "👨‍💻 [Dev: kararAhmed](https://t.me/Kguitaraa)"
        )
...
async def get_web_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Keeps /web alive just in case they lose the code
    if not await check_sub(update, context): return
    # Reuse start logic
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("web", get_web_password))
