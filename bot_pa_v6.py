    ...
    try:
        ...
        if member.status in ['left', 'kicked']:
            ...
            await update.message.reply_text(
                f"⚠️ *عذراً عزيزي!*\n\nلا يمكنك استخدام البوت لـ فحص الأعطال إلا بعد الاشتراك في القناة الرسمية للمطور:\n{CHANNEL_USERNAME}\n\nيرجى الاشتراك ثم المحاولة مرة أخرى للحصول على النتيجة، أو للحصول على الرقم السري للموقع.",
...
async def get_web_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = await update.message.reply_text("⏳ جاري توليد رقم سري آمن مخصص لك...")
    
    new_pass = str(random.randint(100000, 999999))
    try:
        # Save password to Firebase RTDB
        requests.put(f"{DB_URL}/passwords/{new_pass}.json", json={"user_id": user_id, "status": "active"})
        
        # Increment real-time users count
        try:
            count_req = requests.get(f"{DB_URL}/users_count.json")
            curr_count = count_req.json()
            if curr_count is None: curr_count = 15465
            requests.put(f"{DB_URL}/users_count.json", json=(curr_count + 1))
        except: pass # ignore count errors so user still gets pass
        
        await msg.edit_text(
            f"🔑 *رقمك السري الخاص بك هو:*\n\n`{new_pass}`\n\n⚠️ *تنبيه:* هذا الرقم يعمل **لمرة واحدة فقط** ولا يمكن استخدامه من قبل شخصين!\n\nقم بنسخه ولصقه في الموقع لفتحه: [pincfull.web.app](https://pincfull.web.app)",
            parse_mode='Markdown'
        )
    except Exception as e:
        logging.error(f"DB Error: {e}")
        await msg.edit_text("❌ حدث خطأ في النظام، يرجى المحاولة لاحقاً.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "🔑 *للحصول على الرقم السري للموقع أرسل:* /web\n"
        "👨‍💻 *Dev: kararAhmed*",
        parse_mode='Markdown'
    )
