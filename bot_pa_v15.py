...
def analyze(text):
    # Normalize text for 100% accuracy
    t = text.upper().replace("\n", " ").replace("\r", " ")
    t = " ".join(t.split())
    
    # Advanced OCR mistake fixing
    t = t.replace("OX", "0X").replace("0O", "00").replace("I", "1")
    
    keys = sorted(PANIC_DATABASE.keys(), key=len, reverse=True)
    results = [{"code": k, **PANIC_DATABASE[k]} for k in keys if k in t]
    
    # Secondary pass for fuzzy HEX codes via Robust Regex
    if not results:
        hex_match = re.search(r"0X[0-9A-FO]{2,6}", t)
        if hex_match:
            code = hex_match.group().replace("O", "0")
            if code in PANIC_DATABASE:
                results.append({"code": code, **PANIC_DATABASE[code]})

    if not results:
        m = re.search(r"0X[0-9A-F]{2,6}", t)
        if m:
            return f"⚠️ كود غير معروف: `{m.group()}`\nيرجى التواصل مع المطور واتساب لإضافته: 07712782555"
        return "❌ *لم يتم التعرف على الكود بدقة*\nتأكد من وضوح الصورة المرفوعة أو تواصل واتساب: 07712782555"
        
    report = f"🔍 *تقرير تشخيص PincFull — {len(results)} عطل:*\n\n"
    for r in results:
        report += f"🔴 *العطل:* {r['f']}\n🔧 *القطعة:* {r['p']}\n✅ *الإجراء:* {r['s']}\n─────────────\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        msg = (
            f"🔑 *رمز الدخول الخاص بك:* `{new_pass}`\n\n"
            "🌐 *افتح الموقع من هنا أدخل الرمز:*\n"
            "👉 [pincfull.web.app](https://pincfull.web.app)\n\n"
            "أو أرسل صورة العطل هنا مباشرة!"
        )
